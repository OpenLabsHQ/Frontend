...
<div class="flex flex-1 flex-wrap content-start p-4">
  ...
  <div class="flex flex-1 flex-wrap content-start p-4 pl-0">
    ...
    {:else if error}
      <div class="w-full p-4">
        <div
          ...
          role="alert"
        >
          <p class="mb-2 font-bold">We couldn't load templates</p>
      ...
      <div class="w-full p-10 text-center">
        ...
        >
          <div class="mb-4">
            ...
            <h3 class="mb-2 text-xl font-bold">No templates available</h3>
          </div>
          <p class="mb-6 text-blue-700">
            There are no range templates available at the moment. Create a new
    ...
    {:else}
      {#each templates.filter((template) => template.name
          .toLowerCase()
        ...
        >
          <div>
            ...
            <div class="mb-3 flex items-start justify-between">
              ...
              <span
                class={`ml-1 rounded-full px-2 py-1 text-xs whitespace-nowrap ${getProviderInfo(template.provider).color}`}
              >
                {getProviderInfo(template.provider).icon}
                {template.provider}
              </span>
            </div>
            ...
            <p class="mb-4 line-clamp-2 text-sm text-gray-700">
              {template.description ||
                `A ${template.provider} template for creating cyber ranges.`}
            </p>
          </div>
          ...
          <div>
            ...
            <div class="mb-4 flex space-x-2 text-xs">
              <span
                class={`rounded px-2 py-1 ${template.vnc ? 'bg-green-100 text-green-800' : 'bg-gray-100 text-gray-400'}`}
              >
                VNC {template.vnc ? '✓' : '✗'}
              </span>
              <span
                class={`rounded px-2 py-1 ${template.vpn ? 'bg-green-100 text-green-800' : 'bg-gray-100 text-gray-400'}`}
              >
                VPN {template.vpn ? '✓' : '✗'}
              </span>
            </div>
            ...
            <div class="mt-1 flex justify-between">
              ...
              <button
                class="flex items-center rounded-md bg-green-500 px-4 py-2 text-sm font-bold text-white hover:bg-green-700 {deployingTemplateId ===
                template.id
