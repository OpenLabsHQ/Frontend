  ...
  async function deployBlueprint(blueprintId: string) {
    ...
    try {
      ...
      if (result.error) {
        ...
        deploymentSuccess = `Successfully deployed "${blueprint.name}"! You can view it in the Ranges section.`
      ...
      {#if deployingBlueprint}
        <div class="fixed inset-0 z-50 flex items-center justify-center bg-gray-800 bg-opacity-80">
          <div class="rounded-lg bg-white p-8 text-center shadow-xl">
            <h2 class="mb-4 text-xl font-bold text-gray-800">Deploying {blueprint?.name || 'Blueprint'}...</h2>
