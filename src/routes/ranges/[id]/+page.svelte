...
If you encounter any issues, please contact support.`
  ...
  async function loadRangeData() {
    if (browser) {
      ...
      try {
        ...
        // Ensure isRunning property is set (always true as per requirements)
        rangeData.isRunning = true;
  ...
  async function loadSSHKey() {
    try {
      ...
      if (result.error) {
        errorMessage = `Failed to load SSH key: ${result.error}`
  ...
  async function deleteRange() {
    try {
      ...
      // Redirect back to ranges list after a short delay
      setTimeout(() => {
        goto('/ranges')
      }, 1500)
    } catch (err) {
      console.error('Error deleting range:', err)
      deleteError = err instanceof Error ? err.message : 'Failed to delete range'
    } finally {
